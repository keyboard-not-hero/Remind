...
示例5 - 配置MPLS/VPN(PE-to-CE使用OSPF配置, 如图7)
```
配置OSPF
R1(config)# router ospf 1
R1(config-router)# network 10.10.10.0 0.0.0.255 area 0
...
示例6 - route-target import/export方向探讨(参考图7)
```
根据示例5配置, 修改R5的Company_A VRF配置
R5(config)# int f0/1
...
示例7 - 额外添加site与site的通道1(site的router distinguisher在相同的VRF内一致, 参考图7)
```
R1的VRF配置修改结果如下(根据示例5的配置修改)
R1(config)# ip vrf Company_A
R1(config-vrf)# rd 1:100
R1(config-vrf)# route-target both 1:100
...
示例8 - 额外添加site与site的通道2(site的router distinguisher具有唯一性, 参考图7、图8)
```
R1与R5的VRF配置修改结果如下(根据示例5的配置修改)
R1(config)# ip vrf Company_A
R1(config-vrf)# rd 1:100
