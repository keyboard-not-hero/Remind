...
4.将指定网络(connected或IGP)加入BGP table(即Loc-RIB)<br>
`(config-router)# network <network_id> mask <subnet_mask> [route-map <route_map_name>]`<br>
** 网络加入BGP table原理: 将配置的网络在global RIB中的connected或IGP路由中进行搜寻, 能匹配到结果则加入Loc-RIB
<br>
<br>
<br>
...
source_wildcard
使用wildcard模式, 指定可活动的bits. bit值为1代表非固定, 为0代表固定

destination
掩码的bits长度

destination_wildward
使用wildward模式, 指定掩码可活动的bits. bit值为1代表非固定, 为0代表固定
...
代表二进制位11000000 10101|000 00000000 00000000的前13 bits固定, 并且子网掩码在24与28之间的网络
```
<br>
...
(config-route-map)# <match_statement>
```
<br>
...
|match local-preference <local-preference>|根据BGP属性local preference进行过滤|
|match metric {1-4294967295 | external 1-4294967295} [+- deviation]|根据metric进行过滤, 并可以指定误差|
...
|4.如果最后没有permit, 并没有指定match(all prefix)的内容, 默认为deny all|
|5.match后可指定多个具体编号或值, 但逻辑为or|
|6.如果一条route-map语句环境内包含多个match语句, 则逻辑为and|
