...
BGP routing table entry for 192.168.10.0/24, version 19
...
在R4配置route map, 使iBGP R2传播的192.168.10.0/24的weight为200
R4(config)# ip prefix-list PRE192_168_10 permit 192.168.10.0/24
R4(config)# route-map AS65100 permit 10
R4(config-route-map)# match ip address prefix-list PRE192_168_10
R4(config-route-map)# set weight 200
R4(config-route-map)# route-map AS65100 permit 20
R4(config-route-map)# router bgp 65100
R4(config-router)# neighbor 10.1.30.2 route-map AS65100 in
R4(config-router)# end
R4# clear bgp ipv4 unicast 10.1.30.2 soft in
...
Paths: (2 available, best #2, table default)
  ...
  65200 65300
    10.10.10.5 from 10.1.30.2 (2.2.2.2)
      Origin incomplete, metric 0, localpref 100, weight 200, valid, internal, best
      rx pathid: 0, tx pathid: 0x0
...
在R4配置route map, 使iBGP R2传播的192.168.10.0/24的local preference为200
R4(config)# ip prefix-list PRE192_168_10 permit 192.168.10.0/24
R4(config)# route-map AS65100 permit 10
R4(config-route-map)# match ip address prefix PRE192_168_10
...
R4# show bgp ipv4 unicast 10.10.30.0/24
BGP routing table entry for 10.10.30.0/24, version 10
Paths: (2 available, best #2, table default)
  Advertised to update-groups:
     1          2         
  Refresh Epoch 2
