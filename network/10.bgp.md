...
4.在Path Selection中, AS_CONFED_SEQUENCE和AS_CONFED_SET不计入AS_PATH长度统计
<br>
<br>
...
图<br>
![image_not_found](pic/bgp_as_prepend.png)<br>

示例 - route map的prepend AS
```
配置AS 65100的OSPF
R1(config)# router ospf 1
...
R6(config)# router ospf 1
R6(config-router)# network 192.168.0.0 0.0.255.255 area 0
R6(config-router)# network 6.6.6.6 0.0.0.0 area 0

R7(config)# router ospf 1
R7(config-router)# network 192.168.0.0 0.0.255.255 area 0
...
配置BGP
R1(config)# router bgp 65100
R1(config-router)# neighbor 10.10.10.5 remote-as 65200
R1(config-router)# redistribute ospf 1

R5(config)# router bgp 65200
R5(config-router)# neighbor 10.10.10.3 remote-as 65100
R5(config-router)# redistribute ospf 1
...
R5# show bgp ipv4 unicast | begin Network
     Network          Next Hop            Metric LocPrf Weight Path
 *>  1.1.1.1/32       10.10.10.3               2             0 65100 ?
 *>  2.2.2.2/32       10.10.10.3               3             0 65100 ?
 *>  3.3.3.3/32       10.10.10.3               0             0 65100 ?
 *>  4.4.4.4/32       10.10.10.3               2             0 65100 ?
 *>  5.5.5.5/32       0.0.0.0                  0         32768 ?
 *>  6.6.6.6/32       192.168.10.6             2         32768 ?
 *>  7.7.7.7/32       192.168.20.7             2         32768 ?
 *>  8.8.8.8/32       192.168.10.6             3         32768 ?
 *>  172.16.10.0/24   10.10.10.3               2             0 65100 ?
 *>  172.16.20.0/24   10.10.10.3               0             0 65100 ?
 *>  172.16.30.0/24   10.10.10.3               2             0 65100 ?
 *>  172.16.40.0/24   10.10.10.3               0             0 65100 ?
 *>  192.168.10.0     0.0.0.0                  0         32768 ?
 *>  192.168.20.0     0.0.0.0                  0         32768 ?
 *>  192.168.30.0     192.168.10.6             2         32768 ?
 *>  192.168.40.0     192.168.20.7             2         32768 ?



在R5配置1.1.1.1路由prepend AS 65222 65333(从给定的AS list中, 从最末尾的AS开始, 依次prepend到路由的AS path)
R5(config)# ip prefix-list NET_PREFIX seq 10 permit 1.1.1.1/32
R5(config)# route-map AS_PREPEND permit 10
R5(config-route-map)# match ip address prefix-list NET_PREFIX
R5(config-route-map)# set as-path prepend 65222 65333
R5(config-route-map)# route-map AS_PREPEND permit 20
R5(config-route-map)# router bgp 65200
R5(config-router)# neighbor 10.10.10.3 route-map AS_PREPEND in

R5# show bgp ipv4 unicast | begin Network
     Network          Next Hop            Metric LocPrf Weight Path
 *>  1.1.1.1/32       10.10.10.3               2             0 65222 65333 65100 ?
 *>  2.2.2.2/32       10.10.10.3               3             0 65100 ?
 *>  3.3.3.3/32       10.10.10.3               0             0 65100 ?
 *>  4.4.4.4/32       10.10.10.3               2             0 65100 ?
 *>  5.5.5.5/32       0.0.0.0                  0         32768 ?
