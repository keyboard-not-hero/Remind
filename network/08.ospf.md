...
如图14, 当Area 1与Area 3中间间隔一个Area 2时, 由于ABR不允许Type 3 LSA通过非backbone Are接口进入, 所以Area 1和Area 3都没有对方的路由<br>

将R2和R4两个路由器的Area 2接口配置为virtual link, 可以解决该问题<br>

配置方法:
```
(config)# router ospf <process_id>
(config-router)# area <intermediate_area_id> virtual-link <remote_router_id>
```
