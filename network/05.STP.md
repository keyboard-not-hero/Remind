...
cisco模式切换(short模式为802.1d，long模式为802.1t):<br>
`(config)# spanning-tree pathcost method {short | long}`<br>

cisco配置指定vlan的路径cost<br>
`(config-if)# spanning-tree vlan 10 cost 2000`
<br>
<br>

2)当前交换机的邻接交换机网桥ID, 网桥ID小的, 连接该交换机的接口成为root port<br>
cisco配置指定vlan的system priority:<br>
`(config)# spanning-tree vlan <vlan> priority <priority>`
<br>
<br>
...
ciso优先级必须为16的倍数, 默认为128<br>
huawei优先级必须为16的倍数, 默认为128<br>
cisco修改指定vlan的priotity:<br>
