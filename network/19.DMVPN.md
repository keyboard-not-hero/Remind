...
图2<br>
![image_not_found](pic/GRE_route.png)<br>
...
R11(config-router)# router-id 192.168.1.11 
R11(config-router)# network 100.64.11.0 0.0.0.3 area 0

SP(config)# router ospf 1
SP(config-router)# router-id 192.168.1.1
SP(config-router)# network 100.64.11.0 0.0.0.3 area 0
SP(config-router)# network 100.64.31.0 0.0.0.3 area 0

R31(config)# router ospf 1
R31(config-router)# router-id 192.168.1.31
R31(config-router)# network 100.64.31.0 0.0.0.3 area 0


配置GRE
R11(config)# interface tunnel 0
R11(config-if)# tunnel source g0/1
R11(config-if)# tunnel destination 100.64.31.2
R11(config-if)# ip address 192.168.100.11 255.255.255.0

R31(config)# interface tunnel 0
R31(config-if)# tunnel source g0/0
R31(config-if)# tunnel destination 100.64.11.2
...
R12(config-router)# network 10.1.1.0 0.0.0.255

R11(config)# router eigrp 100
R11(config-router)# eigrp router-id 192.168.1.11
R11(config-router)# network 10.1.1.0 0.0.0.255  
R11(config-router)# network 192.168.100.0 0.0.0.255

R31(config)# router eigrp 100
R31(config-router)# eigrp router-id 192.168.1.31
R31(config-router)# network 10.3.3.0 0.0.0.255 
R31(config-router)# network 192.168.100.0 0.0.0.255

R32(config)# router eigrp 100
R32(config-router)# eigrp router-id 192.168.1.32 
R32(config-router)# network 10.3.3.0 0.0.0.255


R11# traceroute 192.168.100.31
Type escape sequence to abort.
Tracing the route to 192.168.100.31
VRF info: (vrf in name/id, vrf out name/id)
  1 192.168.100.31 16 msec 36 msec 24 msec


R12# traceroute 10.3.3.32
Type escape sequence to abort.
Tracing the route to 10.3.3.32
VRF info: (vrf in name/id, vrf out name/id)
  1 10.1.1.11 8 msec 16 msec 16 msec
  2 192.168.100.31 48 msec 16 msec 72 msec
  3 10.3.3.32 48 msec 68 msec 48 msec
...
R8(config-router)# network 192.168.1.8 0.0.0.0
R8(config-router)# network 10.10.10.0 0.0.0.255

R9(config)# router eigrp 100
R9(config-router)# network 192.168.1.9 0.0.0.0
R9(config-router)# network 10.10.0.0 0.0.255.255

R10(config)# router eigrp 100
R10(config-router)# network 192.168.1.10 0.0.0.0
R10(config-router)#network 10.10.20.0 0.0.0.255

R11(config)# router eigrp 100
R11(config-router)# network 192.168.1.11 0.0.0.0
R11(config-router)# network 10.10.30.0 0.0.0.255

R12(config)# router eigrp 100
R12(config-router)# network 192.168.1.12 0.0.0.0
R12(config-router)# network 10.20.10.0 0.0.0.255

R13(config)# router eigrp 100
R13(config-router)# network 192.168.1.13 0.0.0.0
R13(config-router)# network 10.20.0.0 0.0.255.255

R14(config)# router eigrp 100
R14(config-router)# network 192.168.1.14 0.0.0.0
R14(config-router)# network 10.20.20.0 0.0.0.255

R15(config)# router eigrp 100
R15(config-router)# network 192.168.1.15 0.0.0.0
R15(config-router)#network 10.30.10.0 0.0.0.255

R16(config)# router eigrp 100
R16(config-router)# network 192.168.1.16 0.0.0.0
R16(config-router)# network 10.30.0.0 0.0.255.255

R17(config)# router eigrp 100
R17(config-router)# network 192.168.1.17 0.0.0.0
R17(config-router)# network 10.30.20.0 0.0.0.255
...
R12(config-router)# network 172.16.20.0 mask 255.255.255.0
...
R15(config-router)# network 172.16.30.0 mask 255.255.255.0
...
Type:Hub, NHRP Peers:2, 
 ...
 ----- --------------- --------------- ----- -------- -----
     1 172.16.20.12      192.168.100.2    UP 00:16:23     D
     1 172.16.30.15      192.168.100.3    UP 00:16:30     D
...
Type:Spoke, NHRP Peers:1, 
 ...
 # Ent  Peer NBMA Addr Peer Tunnel Add State  UpDn Tm Attrb
 ----- --------------- --------------- ----- -------- -----
     1 172.16.10.8       192.168.100.1    UP 00:16:27     S
...
Type:Spoke, NHRP Peers:1, 
 ...
 # Ent  Peer NBMA Addr Peer Tunnel Add State  UpDn Tm Attrb
 ----- --------------- --------------- ----- -------- -----
     1 172.16.10.8       192.168.100.1    UP 00:15:55     S
...
D        10.10.10.0/24 [90/26882560] via 192.168.100.1, 00:14:26, Tunnel0
D        10.10.20.0/24 [90/26885120] via 192.168.100.1, 00:14:26, Tunnel0
D        10.10.30.0/24 [90/26885120] via 192.168.100.1, 00:14:26, Tunnel0
C        10.20.10.0/24 is directly connected, FastEthernet0/1
L        10.20.10.12/32 is directly connected, FastEthernet0/1
D        10.20.20.0/24 [90/30720] via 10.20.10.13, 00:04:14, FastEthernet0/1
D        10.30.10.0/24 [90/28162560] via 192.168.100.1, 00:04:14, Tunnel0
D        10.30.20.0/24 [90/28165120] via 192.168.100.1, 00:04:14, Tunnel0
      172.16.0.0/16 is variably subnetted, 4 subnets, 2 masks
B        172.16.10.0/24 [20/0] via 172.16.20.5, 00:14:32
C        172.16.20.0/24 is directly connected, FastEthernet0/0
L        172.16.20.12/32 is directly connected, FastEthernet0/0
B        172.16.30.0/24 [20/0] via 172.16.20.5, 00:14:32
      192.168.1.0/32 is subnetted, 10 subnets
D        192.168.1.8 [90/27008000] via 192.168.100.1, 00:14:26, Tunnel0
D        192.168.1.9 [90/27010560] via 192.168.100.1, 00:14:26, Tunnel0
D        192.168.1.10 [90/27013120] via 192.168.100.1, 00:14:26, Tunnel0
D        192.168.1.11 [90/27013120] via 192.168.100.1, 00:14:26, Tunnel0
C        192.168.1.12 is directly connected, Loopback0
D        192.168.1.13 [90/156160] via 10.20.10.13, 00:04:14, FastEthernet0/1
D        192.168.1.14 [90/158720] via 10.20.10.13, 00:04:14, FastEthernet0/1
D        192.168.1.15 [90/28288000] via 192.168.100.1, 00:04:14, Tunnel0
D        192.168.1.16 [90/28290560] via 192.168.100.1, 00:04:14, Tunnel0
D        192.168.1.17 [90/28293120] via 192.168.100.1, 00:04:14, Tunnel0
...
测试ip nhrp shortcut
R12# traceroute 10.30.10.16
Type escape sequence to abort.
Tracing the route to 10.30.10.16
VRF info: (vrf in name/id, vrf out name/id)
  1 192.168.100.1 68 msec 64 msec 72 msec
  2 192.168.100.3 136 msec 100 msec
  3 10.30.10.16 72 msec

R12# traceroute 10.30.10.16
Type escape sequence to abort.
Tracing the route to 10.30.10.16
VRF info: (vrf in name/id, vrf out name/id)
  1 192.168.100.3 72 msec 68 msec 72 msec
  2 10.30.10.16 72 msec 96 msec 88 msec

R12# show ip nhrp
10.30.10.0/24 via 192.168.100.3
   Tunnel0 created 00:00:57, expire 01:59:02
   Type: dynamic, Flags: router rib nho 
   NBMA address: 172.16.30.15

rib nho - 代表由其他路由协议发现的路由, 被NHRP的tunnel IP address替换原来的NBMA IP address
反映在路由表如下(路由前面添加%):
R12# show ip route | include 10.30.10.0
D   %    10.30.10.0/24 [90/28162560] via 192.168.100.1, 00:14:37, Tunnel0

R12# show ip route next-hop-override | section 10.30.10.0
D   %    10.30.10.0/24 [90/28162560] via 192.168.100.1, 00:17:46, Tunnel0
                       [NHO][90/1] via 192.168.100.3, 00:15:29, Tunnel0
...
Gateway of last resort is not set
      ...
      10.0.0.0/8 is variably subnetted, 8 subnets, 2 masks
D        10.10.10.0/24 [90/26882560] via 192.168.100.1, 00:16:22, Tunnel0
D        10.10.20.0/24 [90/26885120] via 192.168.100.1, 00:16:22, Tunnel0
D        10.10.30.0/24 [90/26885120] via 192.168.100.1, 00:16:22, Tunnel0
C        10.20.10.0/24 is directly connected, FastEthernet0/1
L        10.20.10.12/32 is directly connected, FastEthernet0/1
D        10.20.20.0/24 [90/30720] via 10.20.10.13, 00:16:22, FastEthernet0/1
D   %    10.30.10.0/24 [90/28162560] via 192.168.100.1, 00:16:02, Tunnel0
D        10.30.20.0/24 [90/28165120] via 192.168.100.1, 00:16:02, Tunnel0
      172.16.0.0/16 is variably subnetted, 4 subnets, 2 masks
B        172.16.10.0/24 [20/0] via 172.16.20.5, 00:36:33
C        172.16.20.0/24 is directly connected, FastEthernet0/0
L        172.16.20.12/32 is directly connected, FastEthernet0/0
B        172.16.30.0/24 [20/0] via 172.16.20.5, 00:36:33
      192.168.1.0/32 is subnetted, 10 subnets
D        192.168.1.8 [90/27008000] via 192.168.100.1, 00:16:22, Tunnel0
D        192.168.1.9 [90/27010560] via 192.168.100.1, 00:16:22, Tunnel0
D        192.168.1.10 [90/27013120] via 192.168.100.1, 00:16:22, Tunnel0
D        192.168.1.11 [90/27013120] via 192.168.100.1, 00:16:22, Tunnel0
C        192.168.1.12 is directly connected, Loopback0
D        192.168.1.13 [90/156160] via 10.20.10.13, 00:16:22, FastEthernet0/1
D        192.168.1.14 [90/158720] via 10.20.10.13, 00:16:22, FastEthernet0/1
D        192.168.1.15 [90/28288000] via 192.168.100.1, 00:16:02, Tunnel0
D        192.168.1.16 [90/28290560] via 192.168.100.1, 00:16:02, Tunnel0
D        192.168.1.17 [90/28293120] via 192.168.100.1, 00:16:02, Tunnel0
      192.168.100.0/24 is variably subnetted, 3 subnets, 2 masks
C        192.168.100.0/24 is directly connected, Tunnel0
L        192.168.100.2/32 is directly connected, Tunnel0
H        192.168.100.3/32 is directly connected, 00:13:45, Tunnel0
```
<br>
<br>

实例4 - 验证unique特性, 并修改为no-unique
```
1.直接修改NBMA地址
R12(config)# interface tunnel 0
...
2.在NHC修改NBMA地址前使用registration no-unique配置
1)将NBMA地址复原
R12(config)# interface tunnel 0
R12(config-if)# shutdown
R12(config-if)# int f0/0
...
##### IPsec for DMVPN
IPsec组件:<br>
1.Security protocols<br>
1)Authentication Header<br>
提供authentication、integrity和anti-replay
<br>

2)Encapsulation Security Payload(ESP)<br>
提供authentication、integrity、confidentiality和anti-replay
<br>
...
图9<br>
![image_not_found](pic/ESP_encrypto.png)<br>

ESP模式(参考图9):<br>
1.Transport mode<br>
只对除了IP header以外的payload进行封装
<br>

2.Tunnel mode<br>
对包括IP header以内的内容进行封装, 并重新生成一个IP header
<br>
<br>

配置步骤
```
配置security protocol部分(IPsec transform set)
1)配置encryption和authentication
(config)# crypto ipsec transform-set <transform_set_name> [<esp_encryption_name>] [<esp_authentication_name>] [<ah_authentication_name>]

** esp_encryption/esp_authentication/au_thentication_name列表参考表1
** 建议使用esp_encrption_name和esp_authentication_name组合. 推荐esp-aes 256和esp-sha-hmac组合
...
配置key management(IKEv2 Keyring)
1)创建IKEv2 keyring
(config)# crypto ikev2 keyring <keyring_name>

2)创建一个peer
(config-ikev2-keyring)# peer <peer_name>

3)允许加入peer的IP地址
(config-ikev2-keyring-peer)# address <subnet_id> <net_mask>

4)配置pre-shared key
(config-ikev2-keyring)# pre-shared-key <key>



配置Key Associaty(IKEv2 profile)
1)配置IKEv2 profile
(config)# crypto ikev2 profile <ikev2_profile_name>

2)配置允许remote允许加入的peer
(config-ikev2-profile)# match identity remote address <subnet_id>

3)local的接口地址
(config-ikev2-profile)# identity local address <ip_add>
** Pre-Shared Key认证不需要, 用于PKI认证. 最好是loopback 0地址

4)指定Front VRF
(config-ikev2-profile)# match vrf {<vrf_name> | any}
** 当包含FVRF的时候才需要配置

5)local认证方式
(config-ikev2-profile)# authentication local {pre-share | rsa-sig}

6)remote认证方式
(config-ikev2-profile)# authentication remote {pre-share | rsa-sig}

7)本地使用的keyring
(config-ikev2-profile)# keyring local <keyring_name>



配置Key Associaty(IPsec profile)
1)配置IPsec profile
(config)# crypto ipsec profile <ipsec_profile_name>

2)连接transform set
(ipsec-profile)# set transform-set <transform_set_name>

3)连接ikev2 profile
(ipsec-profile)# set ikev2-profile <ikev2_profile_name>



应用到Tunnel接口
(config)# interface tunnel <interface>
(config-interface)# tunnel protection ipsec profile <ipsec_profile_name> [shared]
** 当某个物理接口用于多个encryptied DMVPN tunnel接口时, 必须使用shared参数, 并且tunnel接口此时必须设置tunnel key
```
<br>
<br>

表1 - Transform Matrix<br>
![image_not_found](pic/transform_matrix.png)
<br>
<br>
<br>

anti-replay功能<br>
packet使用sequence number, 并且设置一个window size为X, 当前接收/传送packet的最大sequence number为Y, 则当前允许的sequence number范围[Y-X, Y]
<br>

设置window size(默认值64, 建议值1024)<br>
`(config)# crypto ipsec security-association replay window-size <size>`
<br>
<br>
<br>

Dead Peer Detection(DPD)<br>
IKE/IPsec默认不提供keepalive检测, 当出现故障断开连接后, SA lifetime后自动关闭连接. DPD可以提供检测, 发送R-U-THERE消息, 在等待指定时间N秒没有收到replay后, 在指定间隔时间n秒连续retry 5次后, peer断开<br>
DPD配置在spoke上, 而不配置在Hub上(Hub需要维持所有branch routers的状态)<br>

配置DPD<br>
`(config-ikev2-profile)# crypto ikev2 dpd [<interval>] [<retry_internal>] on-demand`
<br>
<br>
<br>

NAT Keepalive<br>
发送payload为1 byte的UDPpacket, 维持NAT state<br>
配置在Spoke上, 用于维持Spoke-to-Spoke的NAT state<br>

配置NAT Keepalive<br>
`(config)# crypto isakmp nat keepalive <seconds>`
<br>
<br>
<br>
...
实例7 - IPsec(如图4)
```
根据实例3的配置结果, 增加IPsec的相关配置

R8(config)# crypto ipsec transform-set TRANSFORM esp-aes 256 esp-sha-hmac
R8(cfg-crypto-trans)# mode transport
R8(cfg-crypto-trans)# crypto ikev2 keyring KEYRING
R8(config-ikev2-keyring)# peer ANY
R8(config-ikev2-keyring-peer)# address 0.0.0.0 0.0.0.0
R8(config-ikev2-keyring-peer)# pre-shared-key cisco
R8(config-ikev2-keyring-peer)# crypto ikev2 profile IKE_PROFILE
R8(config-ikev2-profile)# match identity remote address 0.0.0.0
R8(config-ikev2-profile)# authentication local pre-share
R8(config-ikev2-profile)# authentication remote pre-share
R8(config-ikev2-profile)# keyring local KEYRING
R8(config-ikev2-profile)# crypto ipsec profile IPSEC_PROFILE
R8(ipsec-profile)# set transform-set TRANSFORM
R8(ipsec-profile)# set ikev2-profile IKE_PROFILE
R8(config)# crypto ipsec security-association replay window-size 1024
R8(config)# interface tunnel 0
R8(config-if)# tunnel protection ipsec profile IPSEC_PROFILE

R12(config)# crypto ipsec transform-set TRANSFORM esp-aes 256 esp-sha-hmac
R12(cfg-crypto-trans)# mode transport
R12(cfg-crypto-trans)# crypto ikev2 keyring KEYRING
R12(config-ikev2-keyring)# peer ANY
R12(config-ikev2-keyring-peer)# address 0.0.0.0 0.0.0.0
R12(config-ikev2-keyring-peer)# pre-shared-key cisco
R12(config-ikev2-keyring-peer)# crypto ikev2 profile IKE_PROFILE
R12(config-ikev2-profile)# match identity remote address 0.0.0.0
R12(config-ikev2-profile)# authentication local pre-share
R12(config-ikev2-profile)# authentication remote pre-share
R12(config-ikev2-profile)# keyring local KEYRING
R12(config-ikev2-profile)# dpd 40 5 on-demand
R12(config-ikev2-profile)# crypto ipsec profile IPSEC_PROFILE
R12(ipsec-profile)# set transform-set TRANSFORM
R12(ipsec-profile)# set ikev2-profile IKE_PROFILE
R12(ipsec-profile)# crypto ipsec security-association replay window-size 1024
R12(config)# crypto isakmp nat keepalive 20
R12(config)# interface tunnel 0
R12(config-if)# tunnel protection ipsec profile IPSEC_PROFILE

R15(config)# crypto ipsec transform-set TRANSFORM esp-aes 256 esp-sha-hmac
R15(cfg-crypto-trans)# mode transport
R15(cfg-crypto-trans)# crypto ikev2 keyring KEYRING
R15(config-ikev2-keyring)# peer ANY
R15(config-ikev2-keyring-peer)# address 0.0.0.0 0.0.0.0
R15(config-ikev2-keyring-peer)# pre-shared-key cisco
R15(config-ikev2-keyring-peer)# crypto ikev2 profile IKE_PROFILE
R15(config-ikev2-profile)# match identity remote address 0.0.0.0
R15(config-ikev2-profile)# authentication local pre-share
R15(config-ikev2-profile)# authentication remote pre-share
R15(config-ikev2-profile)# keyring local KEYRING
R15(config-ikev2-profile)# dpd 40 5 on-demand
R15(config-ikev2-profile)# crypto ipsec profile IPSEC_PROFILE
R15(ipsec-profile)# set transform-set TRANSFORM
R15(ipsec-profile)# set ikev2-profile IKE_PROFILE
R15(ipsec-profile)# crypto ipsec security-association replay window-size 1024
R15(config)# crypto isakmp nat keepalive 20
R15(config)# interface tunnel 0
R15(config-if)# tunnel protection ipsec profile IPSEC_PROFILE



R8# show crypto ipsec transform
Transform set default: { esp-aes esp-sha-hmac  } 
   will negotiate = { Transport,  }, 
   
Transform set TRANSFORM: { esp-256-aes esp-sha-hmac  } 
   will negotiate = { Transport,  }, 



R8# show crypto ikev2 profile
...
IKEv2 profile: IKE_PROFILE
 ...
 Local identity: address 192.168.1.8
...
Interface Tunnel0 is up/up, Addr. is 192.168.100.1, VRF "" 
   Tunnel Src./Dest. addr: 172.16.10.8/MGRE, Tunnel VRF ""
...
----- --------------- --------------- ----- -------- ----- -----------------
    1 172.16.20.12      192.168.100.2    UP 00:07:24     D   192.168.100.2/32
    1 172.16.30.15      192.168.100.3    UP 00:04:45     D   192.168.100.3/32
...
Session: [0xE38F1608]
  Session ID: 2  
  IKEv2 SA: local 172.16.10.8/500 remote 172.16.20.12/500 Active 
          Capabilities:(none) connid:1 lifetime:23:52:35
  Crypto Session Status: UP-ACTIVE     
  fvrf: (none),	Phase1_id: 172.16.20.12
  IPSEC FLOW: permit 47 host 172.16.10.8 host 172.16.20.12 
        Active SAs: 2, origin: crypto map
        Inbound:  #pkts dec'ed 105 drop 0 life (KB/Sec) 4238871/3155
        Outbound: #pkts enc'ed 106 drop 0 life (KB/Sec) 4238871/3155
   Outbound SPI : 0x8BFEFF10, transform : esp-256-aes esp-sha-hmac 
    Socket State: Open
          
Interface: Tunnel0
Session: [0xE38F1700]
  Session ID: 3  
  IKEv2 SA: local 172.16.10.8/500 remote 172.16.30.15/500 Active 
          Capabilities:(none) connid:2 lifetime:23:55:15
  Crypto Session Status: UP-ACTIVE     
  fvrf: (none),	Phase1_id: 172.16.30.15
  IPSEC FLOW: permit 47 host 172.16.10.8 host 172.16.30.15 
        Active SAs: 2, origin: crypto map
        Inbound:  #pkts dec'ed 70 drop 0 life (KB/Sec) 4263573/3314
        Outbound: #pkts enc'ed 69 drop 0 life (KB/Sec) 4263573/3314
   Outbound SPI : 0x11506998, transform : esp-256-aes esp-sha-hmac 
    Socket State: Open
...
interface: Tunnel0
    Crypto map tag: Tunnel0-head-0, local addr 172.16.10.8

   protected vrf: (none)
   local  ident (addr/mask/prot/port): (172.16.10.8/255.255.255.255/47/0)
   remote ident (addr/mask/prot/port): (172.16.30.15/255.255.255.255/47/0)
   current_peer 172.16.30.15 port 500
    ...
    #send errors 0, #recv errors 0
     ...
     local crypto endpt.: 172.16.10.8, remote crypto endpt.: 172.16.30.15
     plaintext mtu 1458, path mtu 1500, ip mtu 1500, ip mtu idb (none)
     current outbound spi: 0x11506998(290482584)
     PFS (Y/N): N, DH group: none

     inbound esp sas:
      spi: 0x7ECCC4DB(2127348955)
        transform: esp-256-aes esp-sha-hmac ,
        in use settings ={Transport, }
        conn id: 3, flow_id: SW:3, sibling_flags 80000000, crypto map: Tunnel0-head-0
        sa timing: remaining key lifetime (k/sec): (4263572/3277)
        IV size: 16 bytes
   ...
   protected vrf: (none)
   local  ident (addr/mask/prot/port): (172.16.10.8/255.255.255.255/47/0)
   remote ident (addr/mask/prot/port): (172.16.20.12/255.255.255.255/47/0)
   current_peer 172.16.20.12 port 500
     PERMIT, flags={origin_is_acl,}
    ...
    #send errors 0, #recv errors 0
     ...
     local crypto endpt.: 172.16.10.8, remote crypto endpt.: 172.16.20.12
     plaintext mtu 1458, path mtu 1500, ip mtu 1500, ip mtu idb (none)
     current outbound spi: 0x8BFEFF10(2348744464)
     PFS (Y/N): N, DH group: none
          
     inbound esp sas:
      spi: 0x5E547C92(1582595218)
        transform: esp-256-aes esp-sha-hmac ,
        in use settings ={Transport, }
        conn id: 1, flow_id: SW:1, sibling_flags 80000000, crypto map: Tunnel0-head-0
        sa timing: remaining key lifetime (k/sec): (4238870/3117)
        IV size: 16 bytes
        replay detection support: Y  replay window size: 1024
        Status: ACTIVE(ACTIVE)
...
[5] IP Authentication Header: https://datatracker.ietf.org/doc/html/rfc4302

[6] IP Encapsulating Security Payload (ESP): https://datatracker.ietf.org/doc/html/rfc4303

[7] Cryptographic Algorithm Implementation Requirements and Usage Guidance for Encapsulating Security Payload (ESP) and Authentication Header (AH): https://datatracker.ietf.org/doc/html/rfc8221

[8] Internet Key Exchange (IKEv2) Protocol: https://datatracker.ietf.org/doc/html/rfc5996

[9] Algorithm Implementation Requirements and Usage Guidance for the Internet Key Exchange Protocol Version 2 (IKEv2): https://datatracker.ietf.org/doc/html/rfc8247
