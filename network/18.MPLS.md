...
图10<br>
![image_not_found](pic/MPLS_PE_TO_CE_OSPF.png)<br>
...
** Downward
在出口处PE生成, 在PE处阻止路由回流造成loop
示例(参考图9/图10): site 3的192.168.110.0/24路由, 依次经过R1 PE/R5 PE到达site 4, 在R5 PE处配置Downward, 当192.168.110.0/24再回流到R5 PE, 并意图通过MPLS区域返回R1 PE时, 被R5 PE阻止. 但Downward不被ABR使用, 如图10, Downward经过R8 ABR(Area Border Router)时被忽略
```
<br>
<br>
...
示例8 - 配置MPLS/VPN(PE-to-CE使用BGP配置, 如图11)
```
配置OSPF
R1(config)# router ospf 1
R1(config-router)# network 10.10.10.0 0.0.0.255 area 0
...
图12 - route-target方向<br>
![image_not_found](pic/vrf_route_target.png)<br>

示例11 - 额外添加site与site的通道2(site的router distinguisher具有唯一性, 参考图7/12)
```
...
示例12 - Hub and Spoke topology(如图13)
```
配置OSPF
R1(config)# router ospf 1
R1(config-router)# network 10.10.10.0 0.0.0.255 area 0
...
R1(config-router-af)# neighbor 172.16.20.6 remote-as 65200
R1(config-router-af)# neighbor 172.16.20.6 activate       
R1(config-router-af)# address-family ipv4 vrf Spoke_2
R1(config-router-af)# neighbor 172.16.30.7 remote-as 65300
R1(config-router-af)# neighbor 172.16.30.7 activate

R6(config)# router bgp 65200
R6(config-router)# neighbor 172.16.20.1 remote-as 65000
R6(config-router)# redistribute connected

R7(config)# router bgp 65300
R7(config-router)# neighbor 172.16.30.1 remote-as 65000
R7(config-router)# redistribute connected
...
配置语法<br>
`(config-router)# neighbor <neighbor_peer> ` 
<br>
...
R6# debug bgp ipv4 unicast update
R6# clear bgp ipv4 unicast *
*Mar 15 04:15:17.519: BGP(0): 172.16.10.1 rcv UPDATE about 7.7.7.7/32 -- DENIED due to: AS-PATH contains our own AS;
*Mar 15 04:15:17.523: BGP(0): 172.16.10.1 rcv UPDATE about 172.16.20.0/24 -- DENIED due to: AS-PATH contains our own AS;

R6(config)# router bgp 65100
R6(config-router)# neighbor 172.16.10.1 allowas-in 1
*Mar 15 04:16:25.067: BGP(0): Revise route installing 1 of 1 routes for 7.7.7.7/32 -> 172.16.10.1(global) to main IP table
*Mar 15 04:16:25.071: BGP(0): Revise route installing 1 of 1 routes for 172.16.20.0/24 -> 172.16.10.1(global) to main IP table

R6# show bgp ipv4 unicast | begin Network 
     Network          Next Hop            Metric LocPrf Weight Path
 *>  6.6.6.6/32       0.0.0.0                  0         32768 ?
 *>  7.7.7.7/32       172.16.10.1                            0 65000 65100 ?
 *>  172.16.10.0/24   0.0.0.0                  0         32768 ?
 *>  172.16.20.0/24   172.16.10.1                            0 65000 65100 ?
```
<br>
<br>
