...
2)指定IPv4协议簇<br>
`(config-router)# address-family ipv4 {unicast | vrf <vrf_name>} autonomous-system <as_number>`<br>
...
```
(config)# router eigrp <as_number>
(config-router)# offset-list <offset_value> {<acl_num> | <acl_name>} {in | out} [<interface_id>]
```
<br>
...
(config-router)# address-family ipv4 unicast autonomous-system <as_number>
(config-router-af)# topology base
(config-router-af-topology)# offset-list <offset_value> {<acl_num> | <acl_name>} {in | out} [<interface_id>]
```
<br>
<br>
