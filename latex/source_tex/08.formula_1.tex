...
%	化学式
%	$\ce{Na(NH4)HPO4 ->[\Delta] (NaPO3)_x + x NH3 ^ + x H2O}$
...
% \indices{right_script}用于显示右方错位角标。包含在tensor宏包中

% \tensor[left_script]{variable}{right_script}用于显示左右方的上下角标，并且上下角标错次排列。该指令包含在tensor宏包中

% \ce{}用于化学式环境，必须包含在公式环境中。包含在mhchem宏包中

% \overline用于上横线，\underline用于下横线

% \overleftarrow用于上方向左箭头，箭头列表：
% overleftarrow/overrightarrow/overleftrightarrow/underleftarrow/underrightarrow/underleftrightarrow。该指令集合包含在amsmath宏包中

% \overbrace{}和\underbrace{}用于上下大括号

% \overbracket[line_width][bracket_height]{math_formula}用于上方中括号。此外\underbracket为下方中括号。包含在mathtools

% \rlap{}将环境内的内容与环境右边的内容进行左对齐重叠

% \phantom{}将环境内的内容进行占位，但不显示

% \frac{molecule}{denominator}用于表示分式

% \tfrac{}{}用于行内公式大小分式，\dfrac{}{}用于行间公式大小分式。包含在amsmath宏包中

% \sfrac为斜式分式。包含在xfrac宏包中

% \binom{}{}用于二项式系数，\tbinom{}{}为行内公式大小二项式系数，\dbinom{}{}为行间公式大小二项式分布。包含在amsmath宏包中
...
% \boldmath可将希腊字母更改为粗体模式，该指令在数学公式范围之外进行指定
