...
\begin{tikzpicture}
    % \draw plot[<options>] (\x,{<y expression>});
    % 使用函数作图. 参数列表:
    %     variable - 指定自变量. 默认为\x
    %     samples - 样本量. 默认为25
    %     domain - 样本区间，格式为<start_val>:<end_val>. 默认为-5:5
    \draw[-latex] (-1,0) -- (3,0);
    \draw[-latex] (0,-3) -- (0,4);
    \draw[smooth,fill=black!20] plot[samples=100,domain=0:1] (\x,{2*(\x-1)*(\x-1)}) -- (0,0) -- cycle;
\end{tikzpicture}
\end{document}
