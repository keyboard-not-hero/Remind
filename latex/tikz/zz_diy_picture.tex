...
\begin{tikzpicture}
    ...
    \node at (1,0.6){6};
    % 转动轮
    \draw[-stealth,line width=0.6pt] (-1,-0.3) -- (1,-0.3);
    \filldraw[rounded corners=2pt] (-1.2,-0.35) rectangle (-0.8,-0.25);
    \filldraw[fill=white,color=white,rounded corners=2pt] (-1.22,-0.36) rectangle (-0.9,-0.24);
    \filldraw[fill=white,rounded corners=2.5pt] (-0.6,-0.4) rectangle (0.6,-0.2);
    \draw[line width=1.2pt,rounded corners=.6pt] (-0.5,-0.3) -- (0.5,-0.3);
    % 规格
    \node at (0,-1){\scriptsize 220V\; 10A(20A)\; 50Hz};
    \node at (0,-1.4){\scriptsize 1\:200\:revs/(kW\:$\boldsymbol{\cdot}$\:h)};
\end{tikzpicture}\vspace{2cm}
